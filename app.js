...
class Streams {
	...
	_locateSegmentRanges(timeStart, timeEnd) {
		let start = this._locateSegment(timeStart);
		let end = this._locateSegment(timeEnd);
		let ranges = [];
	...
	_getFetchRanges(timeStart, timeEnd) {
		let ranges = this._locateSegmentRanges(timeStart, timeEnd);
	...
	fetchMediaSegments(timeStart, timeEnd) {
		let ranges = this._getFetchRanges(timeStart, timeEnd);
		let resbuf = [];
		let fetch = i => {
			let range = ranges[i];
